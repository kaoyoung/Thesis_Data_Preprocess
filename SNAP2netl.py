...
# 平行排序的核心數 (請依據你的 CPU 實際核心數調整)
CPU_CORES = "12"           

# 暫存目錄設定
# 注意：加上 80GB 的排序緩衝，若全放 RAM Disk (/dev/shm) 可能導致 128GB RAM OOM。
# 建議保持在當前目錄 (dir=".")，並確保該目錄位於高速 SSD / NVMe 上。
TEMP_DIR_BASE = "." 
# =============================================================
...
def run_row_net_conversion(input_gz, final_output):
    ...
    try:
        ...
        # 宣告 Hash Map 來記錄新舊 ID 的對應關係 (128GB RAM 處理這個綽綽有餘)
        node_map = {}
        current_new_id = 1
        ...
        with open(step2_sorted, 'r') as fin, open(body_file, 'w') as fbody:
            ...
            for i, line in enumerate(fin):
                parts = line.split()
                # 因為 Step 1 已經強制轉為 1-based 連續編號，這裡直接讀取即可
                net_val = int(parts[0])
                ...
                if net_val != current_net_id:
                    ...
                    # 雖然 re-indexing 理論上不會有 GAP，但保留此邏輯作為安全機制
        ...
        print("="*40)
        print(f"🔹 總節點數 (Nodes): {total_nodes:>15,}  <-- 預期 65,608,366")
        print(f"🔹 總超邊數 (Nets) : {num_nets:>15,}  <-- 預期 65,608,366")
        print(f"🔹 總 Pin 數 (Pins): {final_pins_count:>15,}  <-- 預期 3,677,742,636")
        print("="*40)
